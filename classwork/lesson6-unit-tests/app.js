...
app.controller('MainController', function (Calc) {
	this.doCalculation = (a, b) => {
		this.result = Calc.sum(a, b);
	}
});

app.service('UserService', function ($http) {
	this.getOne = (id) => $http.get(`/users/${id}`).then(response => response.data);
	this.getAll = () => $http.get(`/users/`).then(response => response.data);
});
...
app.component('userCard2', {
	...
	controller: function (UserService) {
		this.someMethod = () => UserService.getOne(1).then(user => this.user = user);
	}
});